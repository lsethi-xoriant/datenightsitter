module Members
  module Provider
    class ProvidersController < ApplicationController
      ...
      def update
        @provider = current_member
        if @provider.update(member_params)
      ...
      def add_seeker
        ...
        unless @seeker.errors.any?
          SmsMessage.createPeerToPeerMsg(@provider, @seeker).invite_to_join
          redirect_to(provider_dashboard_path(@provider),
      ...
      def dashboard
        @provider = current_member
          ...
          if @provider.accepted_tou_at.nil? || @provider.accepted_tou_at < Time.parse(Rails.application.secrets.sittercity_current_tou)   #TODO FIX:  move to config file
            redirect_to provider_terms_of_use_path
          else
            
            @trans = @provider.transactions.order(:started_at => :desc).paginate(:per_page => 3, :page => params[:page])
            @ttl_paid_jobs = @provider.transactions.where(:status => :paid).count
            @ttl_amt = @provider.transactions.where(:status => :paid).pluck(:amount_cents).reduce(0, :+) / 100.to_f
            @ttl_hrs = @provider.transactions.where(:status => :paid).pluck(:duration).reduce(0, :+)
